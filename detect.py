...
def run(
    ...
    for path_rgb, path_fir, im, im0s, vid_cap, s in dataset:
        ...
        # Inference
        visualize = increment_path(save_dir / Path(path_fir).stem, mkdir=True) if visualize else False
        # visualize = increment_path(save_dir / Path(path_FIR).stem, mkdir=True) if visualize else False
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if nchannels == 4:
                ...
                filename_fir = "FIR" + os.sep + str(p_fir.name)
                save_path_rgb = str(Path(out) / Path(filename_rgb))
                save_path_fir = str(Path(out) / Path(filename_fir))
            ...
            if save_img:
                if dataset.mode == "image":
                    if nchannels == 4:
                        ...
                        cv2.imwrite(save_path_fir, img_ir)
                    else:
