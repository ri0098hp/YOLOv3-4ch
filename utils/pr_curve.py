...
for i in range(len(conf_multi2) - 1):
    if recall_multi2[i] == precision_multi2[i]:
        ...
        print(
            f"4 index {i} Conf {conf_multi2[i]:.3f} Precision {precision_multi2[i]:.3f} recall {recall_multi2[i]:.3f}"
        )
for i in range(len(conf_multi3) - 1):
    if recall_multi3[i] == precision_multi3[i]:
        ...
        print(
            f"5 index {i} Conf {conf_multi3[i]:.3f} Precision {precision_multi3[i]:.3f} recall {recall_multi3[i]:.3f}"
        )
...
# )
ax.plot(
    recall_night, precision_night, marker="o", linestyle="solid", color="red", markersize=0, linewidth=1, label="RGB"
)
...
# ax.plot(best_recall_day, best_precision_day,  marker = "D", linestyle="solid", color = "k", markersize=10,
# linewidth=1, label=round(best_recall_day,3))
ax.plot(
    best_recall_night,
    best_precision_night,
    marker="D",
    linestyle="solid",
    color="#CCCC00",
    markersize=10,
    linewidth=1,
    label=round(best_recall_night, 3),
)
# ax.plot(best_recall_multi, best_precision_multi,  marker = "D", linestyle="solid", color = "silver", markersize=10,
# linewidth=1, label=round(best_recall_multi,3))
ax.plot(
    best_recall_multi2,
    best_precision_multi2,
    marker="D",
    linestyle="solid",
    color="teal",
    markersize=10,
    linewidth=1,
    label=round(best_recall_multi2, 3),
)
ax.plot(
    best_recall_multi3,
    best_precision_multi3,
    marker="D",
    linestyle="solid",
    color="tomato",
    markersize=10,
    linewidth=1,
    label=round(best_recall_multi3, 3),
)
