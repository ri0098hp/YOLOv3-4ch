...
def check_train_batch_size(model, imgsz=640, nch=3):
    # Check  training batch size
    with amp.autocast():
        return autobatch(deepcopy(model).train(), imgsz, nch)  # compute optimal batch size


def autobatch(model, imgsz=640, nch=3, fraction=0.8, batch_size=16):
    ...
    try:
        img = [torch.empty(b, nch, imgsz, imgsz) for b in batch_sizes]
